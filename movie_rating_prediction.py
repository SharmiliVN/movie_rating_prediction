...
Original file is located at
    https://colab.research.google.com/drive/1DAuKX7y6XART4GHn_WNaMbsHT_jJJwLp
...
The coefficient of the determination or the R2 score value of Random Forest model is 0.814 which greater than Linear Regression model and it means about 81.4% of the variance in the target variable(Rating) is strongly explained by the model. Also, the mean squared error value or the difference between actual and predicted value is 0.346 and the mean absolute error values is 0.397. Both values seem reasonably low and also both values less than Linear regression model's values. So, we found that the Random Forest Regressor provided significantly more accurate movie rating predictions than the Linear Regression model. The insights gained can help stakeholders in the movie industry make informed decisions regarding film production and marketing strategies.
"""